...
function funksiya(el, fromWatchlist) {
    const movieDiv = document.createElement('div');
    movieDiv.className = 'movie';

    const movieImg = document.createElement('img');
    movieImg.className = 'movie__poster';
    movieImg.src = 'img' + el.poster;
    movieDiv.appendChild(movieImg);

    const movieInfoDiv = document.createElement('div'); //movie div ni yaratamiz
    ...
    h3.className = 'movie__genre';
    h3.innerText = el.genres.join(', ')
    movieInfoDiv.appendChild(h3)

    if (fromWatchlist) {
        const removeButton = document.createElement('button');
        removeButton.className = 'movie__btn remove-btn';
        removeButton.innerText = 'Remove from watchlist';
        removeButton.setAttribute('data-id', el.id);
        removeButton.addEventListener('click', function (event) {
            let store = localStorage.getItem('movies')
            store = store ? JSON.parse(store) : []

            const filterMovies = store.filter(movie => {
                return movie.id !== el.id
            })
            localStorage.setItem('movies', JSON.stringify(filterMovies))
            showAllMovies(filterMovies, true);
        });
        movieInfoDiv.appendChild(removeButton);
    } else {
        ...
        const storeMovie = store.find(movie => {
            return movie.id === el.id
        })
        button.innerText = storeMovie ? 'Remove from watchlist' : 'Add to watchlist';
        if (storeMovie) {
        ...
        button.addEventListener('click', function (event) {
            ...
            const findMovie = store.find(movie => {
                return movie.id === el.id
            })
            if (!findMovie) {
...
if (inputEl) {
    inputEl.addEventListener('input', function (event) {
        let filteredMovies = movies.filter(function (el) {
...
function showAllMovies(arr, fromWatchlist) {
    moviesEl.innerHTML = ''
    for (let i = 0; i < arr.length; i++) {
        funksiya(arr[i], fromWatchlist)
    }
}
...
if (window.location.href.includes('watchlist.html')) {
    ...
    showAllMovies(store, true);
} else {
    showAllMovies(movies, false);
}
